...
class HtmlParser(object):
    def parse_first_floor(self, html_cont, root_url):
        ...
        if html_cont is None:
            logging.warning('[parse_first_floor] skip because of 404! link[%s]' % list_url)
            return None
        soup = BeautifulSoup(html_cont, 'html.parser')
    ...
    def parse_second_floor(self, html_cont, root_url):
        ...
        html_cont = html_downloader.HtmlDownloader().download(list_url)
        if html_cont is None:
            logging.warning('[parse_second_floor] skip because of 404! link[%s]' % list_url)
            return None
        soup = BeautifulSoup(html_cont, 'html.parser')
        urls = soup.select('section > div.ListBox2 > dl > dd > a')
            ...
            if p.text == '':
                ...
                if img:
                    ...
                    try:
                        ...
                        logging.warning('link(%s) img(%s) download fail' % (root_url, img_src))
                    img_no += 1
            else:
