...
class Main(object):
    ...
    def init_log(self):
        ...
        logging.basicConfig(filename='./crawlhc.log', level=logging.DEBUG, format=log_format, datefmt=date_format)
        ...
        try:
            ...
            for first in first_floor:
                ...
                if html_cont is None:
                    logging.info('[collect_category] skip %s %s because of 404!' % (first[0], first[1]))
                    continue
            ...
            traceback.print_exc()
            logging.info('[collect_category] failed')
        finally:
    ...
    def collect_and_crawl(self, pid, cid, link):
        ...
        try:
            logging.info('%s [collect_url] begin!!!' % threading.current_thread().name)
            next_url = link.decode('utf-8')
            has_next = True
            while has_next:
                logging.info('%s [collect_url] crawl' % threading.current_thread().name, pid, '-', cid, next_url)
                html_cont = self.downloader.download(next_url)
                if html_cont is None:
                    logging.info('[collect_url] stop because of 404!')
                    break
            ...
            traceback.print_exc()
            logging.info('%s [collect_url] failed' % threading.current_thread().name)
        finally:
            logging.info('%s [collect_url] end!!!' % threading.current_thread().name)
        ...
        try:
            ...
            while offset < total:
                ...
                for id in url_map:
                    ...
                    if html_cont is None:
                        logging.info('%s [crawl_news] skip because of 404!' % threading.current_thread().name)
                        continue
            ...
            traceback.print_exc()
            logging.info('%s [crawl_news] failed' % threading.current_thread().name)
        finally:
...
if __name__ == '__main__':
    ...
    try:
        ...
        if len(sys.argv) == 1:
            ...
            logging.info('pid[%s]爬取模式' % sys.argv[1])
            main.crawl_by_pid(int(sys.argv[1]))
        elif len(sys.argv) == 3:
            logging.info('pid[%s],cid[%s]爬取模式' % (sys.argv[1], sys.argv[2]))
            main.crawl_by_cid(int(sys.argv[1]), int(sys.argv[2]))
    finally:
        end_time = datetime.datetime.now()
        main.db.dispose()
        logging.info('[main] end!!! %s' % end_time)
        logging.info('共运行%d秒' % (end_time - start_time).seconds)
